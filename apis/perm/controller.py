...
async def perm_lists(page_no: int, page_size: int, search_perm: str = '', db: Session = Depends(get_session),
...
async def add_perm(new_perm: PermBase, db: Session = Depends(get_session)):
    # 查询是否存在
...
async def edit_perm(perm_id: str, perm_edit: PermBase, db: Session = Depends(get_session)):
    # 查询
